...
def calc_M_N_final_d(d):
    ...
    for i in range(len(d)):
        if( (curr_time() >= a[i] ) and (curr_time() < d[i]) and rem_E[i] > 0.001): 
...
while(curr_time() < d[-1]):
    ...
    while(curr_time() >= 5*timer and curr_time() < d[-1]):
        ...
        if first_time:
            ...
            i=0
            old_k = int(np.floor((old_et - old_st) / del_t) )
            if old_k > old_N:
                old_k = N 
            for old_t_idx in old_act_tsk_idx:
                rem_E[old_t_idx] -= (sum(old_W[i,:old_k]) + sum(old_G[i,:old_k]))
                i+=1
                print('remE',rem_E[old_t_idx] )        

        M,N,final_d,act_tsk_idx = calc_M_N_final_d(d)
        ...
        for i in range(M):
            for k in range(N):
                c4_exp = rem_E[i] - sum( m.W[i+1,j+1] + m.G[i+1,j+1] for j in range(k)) - m.energy[i+1,k+1] == 0
                #print(c4_exp)
        ...
        print('Total exec time =', 5*timer + et - st)
        #results.write()
    ...
    M,N,Active_task_final_d,act_tsk_idx = calc_M_N_final_d(d)
    print('remE',rem_E)
