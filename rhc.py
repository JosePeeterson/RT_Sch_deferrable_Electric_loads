# CODE AUTHOR: Jose Peeterson
# PAPER: Real-time scheduling of deferrable electric loads
# SECTION: RHC
    ...
    while(curr_time() >= 5*counter and curr_time() < max(d)):
        ...
        if first_time:
            ...
            for old_t_idx in old_act_tsk_idx: # index from previous active task set
                ...
                print('Wind used = ',sum(old_W[i,:old_k]))
                print('Grid used = ',sum(old_G[i,:old_k]))
                print('current sim time = ',curr_time())
                tot_grid+=sum(old_G[i,:old_k])
                tot_wind+=sum(old_W[i,:old_k])
                i+=1
        ...
        old_et = et
        old_N = N
        old_W = np.zeros((M,N)) # create same dimension zero arrays
        old_G = np.zeros((M,N)) # create same dimension zero arrays
        counter+=1
        old_act_tsk_idx = act_tsk_idx

        # save a copy of current optimzation solution in old_W and old_G
...
print('Sim END time =', curr_time())      
print('tot_grid', tot_grid)
print('tot_wind', tot_wind)
